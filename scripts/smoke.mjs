 ...
 * `waitForTimeout` around a movement assertion is really a frame-rate
 * assertion — it fails on the machine rather than on the mechanic.
 */
...
await awaitPlayerSeconds(0.4);
await page.keyboard.up('Space');
// Long enough for a full jump arc plus the landing, in game time.
await awaitPlayerSeconds(3);
...
// The bound is wall-clock patience, not the claim: the assertion below is still
// "seventy sim-seconds elapsed". A loaded machine simply needs longer to
// deliver them, and failing for that reason says nothing about the game.
...
const fog = await page.evaluate(async () => {
  ...
  const world = getWorld();
  // Fog eases toward its target over several frames, and a software renderer
  // may only manage one or two a second. Wait for frames, not for milliseconds:
  // a fixed sleep here measures the machine rather than the mist.
  const frames = (n) =>
    new Promise((done) => {
      let left = n;
...
const armed = await page.evaluate(() => {
  const w = window.__EDEN__.getWorld();
  // The queue is drained into the displayed line every few seconds, so read
  // both — otherwise this is a race against the HUD rather than a check that
  // ARI said anything.
  const shown = window.__EDEN__.useUI.getState().ariLine ?? '';
  return {
    unlocked: w.player.unlocks.arcBlade,
    equipped: w.player.equipped,
    ari: [...w.ariQueue, shown].join(' | '),
  };
});
...
const spoils = await page.evaluate((creatureId) => {
  ...
  const w = getWorld();
  // Measured as a *change*, not as an absolute. Developer Mode may have handed
  // the tester a stack of fragments already, and "you gained one" is the claim
  // being made here either way.
  const startingFragments = w.player.salvage.coreFragment;
  const c = w.creatures.find((x) => x.id === creatureId);
  combat.damageCreatureByPlayer(w, c, 9999);
  const biologicalSalvage = w.player.salvage.coreFragment - startingFragments;
  ...
  return {
    biologicalSalvage,
    fragments: w.player.salvage.coreFragment - startingFragments,
    chronicle: w.chronicle.slice(-14).map((e) => e.text),
    creaturesLeft: w.creatures.length,
  };
}, encounter.creatureId);
check('a predator leaves no salvage', spoils.biologicalSalvage === 0, `${spoils.biologicalSalvage}`);
